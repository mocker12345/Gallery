...
function Gallery(setting) {
  ...
  if (setting && Object.keys(setting).length >0) {
    ...
    if (this.setting.speed == 'fast') {
      this.divisor = 2;
    } else if (this.setting.speed == 'slow') {
      this.divisor = 5;
    } else {
      this.divisor = 2;
      console.error('参数错误');
  ...
  window.onresize = function () {
    self.loadSize(self.groupDate[self.index].src);
  }
}
  ...
  move: function startMove(obj, json, fun) {
    ...
    obj.timer = setInterval(function () {
      ...
      for (var name in json) {
        ...
        var speed = (json[name] - cur) / self.divisor;
